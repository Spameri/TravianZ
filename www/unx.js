...
function qst_next2(jf, act, act2, act3) {
    ...
    if (jf) {
        id = {'x': document.getElementById('qst_val_x').value, 'y': document.getElementById('qst_val_y').value};
    }

    else {
        id = {'val': document.getElementById('qst_val').value};
    }
...
function executeTimer(zb) {
    ...
    if (wb >= 0) {
        console.log(timer[zb].production);
...
function add_res(gc) {
    ...
    document.getElementById('r' + gc).value = fc[gc];
}
function upd_res(gc, kc) {
    hc = fb['l' + (5 - gc)].value;
    ic = haendler * carry;
    if (kc) {
        lc = hc;
...
function btm1() {
    ...
    for (var i = 0; i < (a.length - 2); i += 3)if ((x = rc(a[i])) != null) {
        document.MM_sr[j++] = x;
...
function yg(ah, bh, _e) {
    ...
    var area = pe(ah, bh, 'a');
    te(_e, area);
    of.className = area.details.img;
    pf(area, of);
}
function pf(area, of) {
    if (area.details.atyp) {
        if (!of.firstChild) {
            of.appendChild(document.createElement('span'));
        }
...
function hh() {
    $e({$h: '', name: '', _h: '', ai: '', x: m_c.z.x, y: m_c.z.y}
    );
}
...
function ii(mi) {
    ...
    if (mi) {
        var oi = '<div class="popup3 quest"><a href="#" onClick="qst_handle()"><img src="img/x.gif" border="1" class="popup4" alt="Close"></a><div id="popup3"</div></div>';
        ni.innerHTML = oi;
        pi();
        qst_wfm();
        vc();
...
function vil_levels_toggle() {
    var cj = $('levels'), dj = $('lswitch');
    cj.toggleClass('on');
    dj.toggleClass('on');
    if (cj.hasClass('on')) {
        document.cookie = 't3l=1; expires=Wed, 1 Jan 2020 00:00:00 GMT';
    }
    else {
        document.cookie = 't3l=1; expires=Thu, 01-Jan-1970 00:00:01 GMT';
    }
}
function gmwd() {
